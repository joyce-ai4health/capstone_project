...
# -------------------------------
def total_seller_revenue(sales_with_price, seller_id):
    return sales_with_price[sales_with_price['seller_id']==seller_id]['total_price'].sum()

def top_seller_products(sales_with_price, seller_id, n=3):
    return sales_with_price[sales_with_price['seller_id']==seller_id].groupby('food_id')['quantity_sold']         .sum().sort_values(ascending=False).head(n)

def low_demand_products(sales_with_price, seller_id, n=3):
    return sales_with_price[sales_with_price['seller_id']==seller_id].groupby('food_id')['quantity_sold']         .sum().sort_values(ascending=True).head(n)

def monthly_revenue_trend(sales_with_price, seller_id):
    return sales_with_price[sales_with_price['seller_id']==seller_id]         .groupby(['year','month'])['total_price'].sum().reset_index()

def plot_monthly_trend(monthly_df, seller_id):
    plt.figure(figsize=(8,4))
    plt.plot(monthly_df['month'], monthly_df['total_price'], marker='o')
    plt.title(f"Seller {seller_id} Monthly Revenue Trend")
    plt.xlabel("Month")
    plt.ylabel("Revenue")
    plt.grid(True)
    plt.show()

def seller_dashboard(sales_with_price, seller_id):
    print(f"--- Seller {seller_id} Dashboard ---")
    print("Total Revenue:", total_seller_revenue(sales_with_price, seller_id))
    print("\nTop 3 Products:\n", top_seller_products(sales_with_price, seller_id))
    print("\nLow Demand Products:\n", low_demand_products(sales_with_price, seller_id))
    monthly_df = monthly_revenue_trend(sales_with_price, seller_id)
    print("\nMonthly Revenue Trend:\n", monthly_df)
    plot_monthly_trend(monthly_df, seller_id)
...
def prepare_ml_data(sales_with_price):
    ...
    for seller_id in monthly_seller_revenue['seller_id'].unique():
        ...
        monthly_seller_ml_data[seller_id] = {'X_train':X_train_s,'X_test':X_test_s,'y_train':y_train_s,'y_test':y_test_s}
    # Daily
    ...
    for seller_id in daily_seller_revenue['seller_id'].unique():
        ...
        if len(X_seller)>1:
            ...
            X_train_s = X_test_s = X_seller
            y_train_s = y_test_s = y_seller
        daily_seller_ml_data[seller_id] = {'X_train':X_train_s,'X_test':X_test_s,'y_train':y_train_s,'y_test':y_test_s}
